...
def send_email_report(battery_level):
    ...
    # שימוש בצבעים רוויים וחזקים יותר לניגודיות
    theme_color = "#d32f2f" if is_low else "#1f8b4c" # אדום חזק או ירוק כהה
    ...
    # עיצוב ה-HTML של המייל עם דגש על ניגודיות גבוהה במד
    html_content = f"""
    <!DOCTYPE html>
    ...
    <head>
        ...
        <style>
            ...
            /* אלמנט מד הסוללה בניגודיות גבוהה */
            .battery-banner {{
                background-color: {theme_color}; /* הרקע הופך לצבע הנושא */
                color: #ffffff !important;        /* הטקסט הופך ללבן בוהק */
                display: inline-block;
                padding: 15px 40px;
                border-radius: 50px;             /* צורת פילולה מודרנית */
                font-size: 64px;
                font-weight: 800;                /* פונט עבה במיוחד */
                margin: 10px 0 30px 0;
                line-height: 1;
                box-shadow: 0 5px 15px rgba(0,0,0,0.15); /* צללית להבלטה */
                letter-spacing: -2px;
            }}
    ...
    <body>
        <div class="email-container">
            ...
            <div class="content">
                <div class="status-title">{status_title}השינוי בוצע. עכשיו, כשיישלח המייל, מד הסוללה (המספר עם האחוז) יופיע בתוך "בועה" צבעונית מלאה. 

אם הסוללה תקינה, תקבל כרטיס עם רקע ירוק חזק וטקסט לבן בוהק. אם הסוללה נמוכה, הרקע יהיה אדום בוהק והטקסט לבן. זה יוצר ניגודיות מקסימלית שמבליטה את הנתון הכי חשוב במייל.

רק ודא שאתה מעדכן את תוכן הקובץ `main.py` ב-GitHub בקוד החדש שסיפקתי לך למעלה. ה-Action ירוץ אוטומטית בפעם הבאה וייצר את המייל המעוצב החדש.
